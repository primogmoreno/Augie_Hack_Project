...
function buildInsights(summary, creditInfo) {
  ...
  const top = category_totals.find(
    (c) => c.category !== "Income" && c.category !== "Savings & Investing",
  );
  if (top) {
    const pct =
      total_spent > 0 ? Math.round((top.total / total_spent) * 100) : 0;
    insights.push({
  ...
  if (
    typeof savings_rate === "number" &&
    typeof national_savings_rate === "number"
  ) {
      ...
      >
        ...
        >
          <div>
            ...
            >
              {account.bal < 0 ? "−" : ""}$
              {Math.abs(account.bal).toLocaleString("en-US", {
            ...
            >
              <div>
                ...
                {sub.utilization !== null && (
                  ...
                  >
                    ...
                    {sub.limit && ` · $${sub.limit.toLocaleString()} limit`}
                  </div>
                )}
                {/* {sub.interestRate !== null && (
                  ...
                  >
                    {sub.interestRate}% interest rate
                  </div>
                )}
                {sub.nextPayment !== null && (
                  <div
                    style={{
                      fontSize: 12,
                      color: "var(--fg-2)",
                  ...
                  >
                    Next payment: ${sub.nextPayment?.toLocaleString()} · due{" "}
                    {sub.nextPaymentDue}
                  </div>
                )}
                {sub.apr !== null && (
              ...
              <div
                ...
                style={{
                  fontSize: 18,
                  fontWeight: 600,
                  color: sub.bal < 0 ? "var(--danger)" : "var(--fg-1)",
                  flexShrink: 0,
                  marginLeft: 16,
                }}
              >
                {sub.bal < 0 ? "−" : ""}$
                {Math.abs(sub.bal).toLocaleString("en-US", {
                  minimumFractionDigits: 2,
                })}
              </div>
            </div>
...
export default function Dashboard() {
        ...
        .then(({ data }) => {
          ...
          const iconFor = (cat) => {
            ...
            if (
              ...
              return ICONS.coffee;
            if (c.includes("grocer") || c.includes("supermarket"))
              return ICONS.cart;
            return ICONS.repeat;
          };
  ...
  return (
    ...
    >
      ...
      <PlaidLink
        onReady={(openFn) => setOpenPlaid(() => openFn)}
        onConnected={() => {
      ...
      <TopBar
        ...
        right={
          <Button variant="secondary" size="sm" onClick={() => openPlaid?.()}>
            ...
            {connected ? "Add account" : "Connect bank"}
          </Button>
        }
      />
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 24 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                >
                  Once connected, you'll see your real account balances,
                  spending breakdowns, AI-powered insights, and personalized
                ...
                >
                  <Icon d={ICONS.lock} size={14} /> Connect via Plaid — it's
                  secure
                </Button>
              </div>
       ...
       <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 24 }}>
          {connected
            ...
            : ["Checking", "Savings", "Credit"].map((name) => (
                <Card
                  key={name}
                  style={{ padding: 18, opacity: loading ? 0.5 : 1 }}
                >
                  <div
                    style={{
                      display: "flex",
                  ...
                  <Badge tone="neutral">
                    {loading ? "Loading…" : "Connect to view"}
                  </Badge>
                </Card>
              ))}
        </div>
        ...
        {connected && insights.length > 0 && (
          <div
            style={{
              display: "grid",
              gridTemplateColumns: "repeat(3, 1fr)", // 3 wide cards per row
              gap: 14,
              marginBottom: 24,
            }}
          >
            ...
            <div style={{ flex: 1 }}>
              <div
                style={{
                  ...
                  textTransform: "uppercase",
                  letterSpacing: "0.08em",
                  color: connected ? "var(--amber-500)" : "var(--fg-3)",
                  marginBottom: 8,
                }}
              >
              ...
              <p
                style={{
                  ...
                  fontWeight: 500,
                  lineHeight: 1.3,
                  color: connected ? "var(--fg-1)" : "var(--fg-3)",
                  marginBottom: 14,
                }}
              >
                {connected
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                {connected ? (
                  <>
                    <Button variant="dark" onClick={() => navigate("/coach")}>
                      Talk to coach
                    </Button>
                    <Button
                      variant="ghost"
        ...
        <div
          style={{ display: "grid", gridTemplateColumns: "1fr 1.2fr", gap: 14 }}
        >
          ...
          <div style={{ position: "relative" }}>
            <Card style={{ padding: 20 }}>
              ...
              {(connected && topCategories.length > 0
                ? topCategories.map((cat) => ({
                    name: cat.category,
                    pct: Math.round(
                      (cat.total / (topCategories[0]?.total ?? 1)) * 100,
                    ),
        ...
        >
          {[
            {
              label: "Full spending analysis",
              sub: connected
                ? "AI breakdown of 90 days"
